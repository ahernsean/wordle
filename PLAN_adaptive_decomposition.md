...
```
_solve_subset(X)                          worker publisher (mid_loop_publisher)
(wordle_engine.py)                        (erd_swarm.py)
├─ entry: token = enter(n)
│  nodes_at_entry, predicted=c(n)
├─ inline best-first loop
│  (Σk² order, builds best_erd)
│
└─→ check(token): nodes-delta            ┌─ map evaluated words → all_words idx
    > K*predicted AND > floor?          │  create_branch (PROMOTED_PRIORITY)
    │                                    │  seed best_erd, mark prefix done
    │                                    │
    └─→ YES: overrun trip ────────────→ │─ drive remainder:
           (Phase 2)                      │  claim_candidate (natural order)
                                         │  + _help_other_branch + maybe_finalize
    OR                                   │
subbranch_solver hook                    │
(entry, c(size) confident-huge)          │─ return min(partial, cooperative)
(Phase 1: a-priori) ───────────────────→└─ engine short-circuits

cost_model                                 cost_model
(erd_queue.sqlite3)                        (erd_queue.sqlite3)
├─ inline return sample (n, Δnodes)   ← ─ phase 1, 2 feed samples
└─ finalize sample (n_words, nodes_spent)
```

- **Phase 1 — foundation.** Cost model `c(size)` (table, time-decay, exponential
  weighting with ~1-day half-life, cold-start stochastic probing with `2 × worker_count`
  samples, in-memory cache, read/update); node sampling (inline returns + cooperative
...
- `erd_queue.py` — `cost_model` table + read/update; `nodes_spent` column; full
  ...
  sizing; heartbeat column renames; `claim_telemetry` table.
...
- **Unit** (`test_erd_queue_unit.py`, `test_erd_swarm_unit.py`, `test_erd_parallel.py`,
  ...
  written with sane `coordination_nanos`/`work_nodes`.
  Update/remove the `chunk_size_for` tests (test_erd_queue_unit.py:55-67,
