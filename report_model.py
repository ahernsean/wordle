...
class ReportFilters:
    ...
    branch_worker_statuses: tuple[str, ...] = ()
...
def branch_status_and_worker_status(
    ...
    """Derive a branch's lifecycle stage and, when a worker could currently
    be on it, whether one is."""
    ...
    return status, ("active" if worker_count else "waiting")
...
def _normalize_branch(
    row, branch_status, branch_worker_status, progress, worker_count, answer_set,
):
...
def worker_state(worker, generated_at, branch_status):
    ...
    each re-deriving it.  branch_status is the status of the worker's current
...
def _queue_overview(sources, generated_at, answer_set, report):
    ...
    try:
        ...
        active_branch_keys = [
            ...
            if row["branch_worker_status"] == "active"
        ]
        ...
        status_by_key = {
            branch["branch_key_hex"]: branch["branch_status"]
            for branch in normalized_rows
        }
        for worker in workers:
            worker["state"] = worker_state(
                worker, generated_at, status_by_key.get(worker["branch_key_hex"])
            )
...
def _filters_payload(filters):
    return {
        ...
        "branch_worker_statuses": list(filters.branch_worker_statuses),
...
def collect_word_report(sources: ReportOpeners, request: ReportRequest) -> dict:
    ...
    for group_row in group_rows:
        ...
        branch_status, branch_worker_status = branch_status_and_worker_status(
    ...
    if filters.branch_worker_statuses:
        response_groups = [
            row for row in response_groups
            if row["branch_worker_status"] in filters.branch_worker_statuses
        ]
...
def collect_root_progress_report(sources: ReportOpeners,
    ...
    for pattern, answer_count in sorted(answer_counts.items()):
        ...
        # The spine that finalized the branch, which is where the work
        # actually happened: a branch is solved once and read from cache
        # afterwards, so its finalize row is the only record there is.
        #
        # A spine of one guess is itself the proof that the branch is that
        # opener's response group -- the row was written at that node -- so
        # the pair names the branch and colours the tiles.  A deeper spine
        # means no single guess of that opener isolates this answer set, and
        # the opener stands alone: QAJAQ reaches {sheet, skeet, sweet} only
        # after two more guesses, and its own first response holds nineteen
        # hundred answers.
        payer_steps = (_spine_steps(finalizing_spine)
                       if row["provenance"] == "inherited" else ())
        row["paid_by_spine"] = [_step_payload(step, answer_set)
                                for step in payer_steps]
        row["paid_by_guess_depth"] = len(payer_steps)
        row["paid_by_pattern"] = (
            payer_steps[0].pattern if len(payer_steps) == 1 else None)
...
def collect_branch_report(sources: ReportOpeners, request: ReportRequest) -> dict:
    ...
    for worker in workers:
        worker["state"] = worker_state(worker, generated_at, branch_status)
    progress = {
...
def collect_workers_report(sources: ReportOpeners, request: ReportRequest) -> dict:
    ...
    try:
        ...
        filters_have_branch_scope = any((
            ...
            request.filters.branch_worker_statuses,
        ...
        status_by_key = {
            row["branch_key_hex"]: row["branch_status"] for row in scoped_rows
        }
        by_state = {
            "working": 0,
            "coordinating": 0,
...
def _opener_rollups(membership_rows):
    ...
    for row in membership_rows:
        ...
        branch_status, _branch_worker_status = branch_status_and_worker_status(
