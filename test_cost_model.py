...
class TestSizeBucketing(_TmpQueue):
    ...
    def test_unseen_size_borrows_from_bucket_mate(self):
        ...
        self.assertIsNone(self.q.get_cost_typical(POLICY, 100))
        self.q.update_cost_model(POLICY, 100, 4000)
        self.assertIsNotNone(self.q.get_cost_typical(POLICY, 105))
        self.assertEqual(self.q.get_cost_typical(POLICY, 100),
                         self.q.get_cost_typical(POLICY, 105))
        ...
        if cost_size_bucket(130) != cost_size_bucket(100):
            self.assertIsNone(self.q.get_cost_typical(POLICY, 130))
...
class TestGeometricMeanConverges(_TmpQueue):
    def test_converges_to_geometric_mean(self):
        ...
        for x in _lognormal_samples(mu, sigma, 2000, seed=1):
            ...
            self.q.update_cost_model(POLICY, 137, x, now=now)
        typical = self.q.get_cost_typical(POLICY, 137)
    ...
    def test_robust_to_single_tarpit(self):
        ...
        now = 1_000_000
        for _ in range(200):
            self.q.update_cost_model(POLICY, 50, 1000, now=now)
        self.q.update_cost_model(POLICY, 50, 10_000_000, now=now)
        typical = self.q.get_cost_typical(POLICY, 50)
...
class TestTimeDecay(_TmpQueue):
    def test_old_regime_fades(self):
        ...
        t0 = 1_000_000
        for _ in range(20):
            self.q.update_cost_model(POLICY, 80, 1_000_000, now=t0)
        # Jump ~10 half-lives forward, then stream the cheap regime.
        t1 = t0 + int(10 * _COST_MODEL_TAU)
        for i in range(50):
            self.q.update_cost_model(POLICY, 80, 100, now=t1 + i)
        typical = self.q.get_cost_typical(POLICY, 80)
    ...
    def test_no_decay_within_same_instant(self):
        ...
        now = 5_000
        for _ in range(10):
            self.q.update_cost_model(POLICY, 80, 500, now=now)
        self.assertAlmostEqual(self.q.get_cost_typical(POLICY, 80), 500, delta=2)
...
class TestSpread(_TmpQueue):
    def test_sigma_recovered_from_second_moment(self):
        ...
        now = 1_000_000
        a, b = 100, 10_000
        self.q.update_cost_model(POLICY, 60, a, now=now)
        self.q.update_cost_model(POLICY, 60, b, now=now)
        expected = abs(math.log(a) - math.log(b)) / 2
        self.assertAlmostEqual(self.q.get_cost_spread(POLICY, 60), expected,
                               places=6)

    def test_zero_spread_for_identical_samples(self):
        now = 1_000_000
        for _ in range(5):
            self.q.update_cost_model(POLICY, 60, 777, now=now)
        self.assertAlmostEqual(self.q.get_cost_spread(POLICY, 60), 0.0, places=6)

    def test_spread_matches_lognormal_sigma(self):
        mu, sigma = math.log(3000), 0.9
        now = 1_000_000
        for x in _lognormal_samples(mu, sigma, 3000, seed=7):
            self.q.update_cost_model(POLICY, 200, x, now=now)
        self.assertAlmostEqual(self.q.get_cost_spread(POLICY, 200), sigma,
                               delta=0.06)
...
class TestBatchEqualsIndividual(_TmpQueue):
    def test_logsums_batch_matches_one_at_a_time(self):
        ...
        now = 1_000_000
        for x in samples:
            self.q.update_cost_model("indiv", 90, x, now=now)
        ...
        self.assertAlmostEqual(self.q.get_cost_typical("indiv", 90),
                               self.q.get_cost_typical("batch", 90), places=6)
        self.assertAlmostEqual(self.q.get_cost_spread("indiv", 90),
                               self.q.get_cost_spread("batch", 90), places=6)
    ...
    def test_small_geometric_mean_not_floored_to_one(self):
        ...
        typical = self.q.get_cost_typical(POLICY, 12)
...
class TestPolicyAndBucketIsolation(_TmpQueue):
    def test_policy_keeps_models_separate(self):
        now = 1_000_000
        self.q.update_cost_model("erd_all", 100, 200, now=now)
        self.q.update_cost_model("erd_answers", 100, 9000, now=now)
        self.assertAlmostEqual(self.q.get_cost_typical("erd_all", 100), 200,
                               delta=1)
        self.assertAlmostEqual(self.q.get_cost_typical("erd_answers", 100), 9000,
                               delta=1)
